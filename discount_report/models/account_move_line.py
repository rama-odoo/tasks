...
class AccountMoveLine(models.Model):
    ...
    def _get_price_total_and_subtotal(self, price_unit=None, quantity=None, discount=None, currency=None, product=None, partner=None, taxes=None, move_type=None):
        ...
        res = super()._get_price_total_and_subtotal(price_unit=price_unit, quantity=quantity, discount=discount,
                                                    currency=currency, product=product, partner=partner, taxes=taxes, move_type=move_type)
        print("***res *** : ", res.get('price_subtotal'))
        print("2 discount,", self.discount_2)
        price_subtotal = res.get('price_subtotal') - \
            ((res.get('price_subtotal')*self.discount_2)/100)
